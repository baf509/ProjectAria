...
The deployed source and verification records are committed on `tool-repairs-20260915` in `ProjectAria-tool-repairs`. Reconcile this branch with canonical `master` before deploying from master again. Master contains unrelated unfinished changes, including model registry/MCP edits, so it was not overwritten or merged during cleanup. Keep the active source worktree and rollback backups until that integration is complete.
