...
| Qwen3.8 Flash Next UD-Q4_K_XL hybrid | R9700 dense/KV/MTP + Strix Halo experts | `127.0.0.1:8121` | alternate whole-machine loadout; replaces both resident servers |
| Gemma 4 E4B Q4 | Mac native | `127.0.0.1:8104` | auxiliary workers and side tasks |

The normal loadout is Radiance on `:8080` plus Halo-only Flash Next on `:8120`.
The hybrid service is disabled at boot and its systemd unit conflicts with both
normal services. Switch between these profiles through the dashboard loadout
...
nc -z 127.0.0.1 8080
nc -z 127.0.0.1 8120
# Present as an SSH listener even while the alternate service is stopped.
nc -z 127.0.0.1 8121

# Corsair observed state
systemctl --user is-active qwen3.8-radiance.service qwen3.8-flash-next.service
systemctl --user is-active qwen3.8-flash-next-hybrid.service || true
