...
00. **Pi-Flow parity — implemented (2026-07-25, branch `feat/piflow-parity`).** Closed the gaps vs [pi-flow](https://github.com/kky42/pi-flow): a global concurrency limiter + queue for coding sub-agents (`coding_max_concurrent_sessions`, `queued` state, `GET /coding/sessions/concurrency`), a `wait_for_session` join primitive, fan-out workflow actions (`parallel`/`map`/`code_session await`/`synthesize` with nested `{{steps.N.results.M}}` interpolation), prompt-cache metrics (`cache_hit_rate` on `/usage/*`), and declarative specialist profiles (`subagent_profile`). MCP gained workflow tools + concurrency/cache in `fleet_status`. Plan: `vault/ProjectAria/Planning/PiFlow_Parity_Plan.md`; 873 tests pass. **Next: restart `aria-api` + `hermes-gateway.service`; verify a fan-out workflow end-to-end.**

0. **Complexity routing — LIVE on the automated spawn path (2026-07-24).** Unpinned coding tasks spawned via `start_session()` (Hermes/MCP/TUI create) are classified into a tier and run on that tier's model — planning/design → Opus 4.8, scoped work → Sonnet 5, research → Sonnet 5; Sonnet is the floor, quota fallback → local open-weights server. Code in `api/aria/agents/routing.py`, `api/aria/api/routes/routing.py`; 61 tests. Verified live: an unpinned design prompt spawned a real Claude Code session on `claude-opus-4-8` with the verdict persisted; `backend=codex` correctly stayed unrouted. **Desk-path auto-routing was tried and deliberately dropped (see CLAUDE.md → Desk path):** Claude Code is one-model-per-session, so the interactive REPL has no task to classify and can't be re-routed mid-session. Bare `claude` is the saved-state attach; routing is spawn-path only. Scripts kept but unsourced. **Done — no follow-up.**
