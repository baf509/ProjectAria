...
**Current model topology** (the agents are config rows in `db.agents` — read them, don't trust this list blindly; as of 2026-07-28, two-server split + same-day routing correction — full detail in `docs/ops/LOCAL_INFERENCE_TOPOLOGY.md`):
- **Two local model servers, no shared KV cache between them.** `chadrock` (`:8102`, Laguna S 2.1 ROCmFP4/Vulkan) is the **`pool` CLI's dedicated server only** — nothing else may point at it, on purpose, since it's `--parallel 1`. `qwen3.6-35b-a3b` (`:8103`, Qwen3.6-35B-A3B-MTP ROCmFP4/Vulkan, renamed from `qwen-hermes` once it stopped being Hermes-only) serves **Hermes** plus **ARIA's default chat agent and Search Agent** (`backend=llamacpp`/`agentic` both resolve here).
