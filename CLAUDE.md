...
- **The Halo side, one at a time (86–100 GiB each).** `DS4-0731-IQ3_XXS-Halo-Vulkan` (`:8108`, `ds4-halo-xxs.service`, Nathan's Vulkan fork — it implements the DeepSeek-V4 kernels mainline Vulkan disables, which is why it beats both mainline Vulkan and mainline ROCm here) is the **APU-only** profile — since 2026-08-15T16:35 the **single-slot coding-agent (pi) model** (q8_0 KV, one 131K slot, no drafter; Hermes's default moved to Qwen3.8 on the R9700). `DS4-0731-IQ3_S-Hybrid-ROCm-Dual` (`:18211`, mainline HIP dual-arch) **splits the higher-quality IQ3_S across both cards** (80/20, `PLACEMENT=split|hybrid`). `DS4-0731-ROCmFPX-Affine-Quality` (`:8107`, sealed O5 runtime) is the **quality/long-recall reference** — 238/256 broad, 24/24 long-context, and slow; type-108 tensors load on that runtime ONLY. **`gemma-aux`** (`:8104`, Gemma 4 E4B Q4_0, **CPU-only**) takes Hermes's ~16 auxiliary side-tasks + crons plus ARIA's shell- and ontology-extraction workers, and coexists with anything.
  - **`DS4-0731-Q8Protected-Halo-DwarfStar`** (`:8112`, `infrastructure/dwarfstar-ds4/serve.sh`, `runtime_family="dwarfstar"`) is the **selected successor** to the IQ3_XXS Vulkan profile (six-way bakeoff, `vault/infrastructure/Analysis/DS4_STACK_BAKEOFF_20260817.md`) — native ROCm via antirez's DwarfStar, ~40 s loads. ⚠️ **Deployed and registered but NOT cut over** (2026-08-17): pi, Hermes and ARIA still point at `:8108`. ⚠️ **~100 GiB measured GTT** despite its own 82 GiB self-report — a wash with IQ3_XXS, and it exposes `/v1/models` only (no `/metrics`, `/slots` or `/health`), so utilization reports `telemetry_hint` rather than numbers.
- **The R9700 side, one at a time (~29 GiB).** ⚠️ **Since 2026-08-16 this side is vLLM, not llama.cpp.** `Qwen3.8-27B-R9700-Radiance` (`:8080`, `qwen3.8-radiance.service`, `infrastructure/qwen3.8-radiance/serve.sh`) serves **int4 W4A16 (AutoRound) weights through vllm-radiance 0.5.8**, multimodal, **196608 ctx × 1 slot**, 29 GiB VRAM. It is **Hermes's DEFAULT model** (`custom:qwen38-r9700`, declared **196608**) and serves **two aliases** — `qwen3.8-27b-r9700` for Hermes's main provider and `qwen3.8-27b-rocmfp4-r9700` for Hermes's auxiliary roles and ARIA's `config.steward_model` (a misnomer now, kept so the cutover broke nothing). DS4 on `:8108` remains the single-slot **coding-agent (pi)** model.
  - **Why it replaced the GGUF path** (measured 2026-08-16, same card, wikitext-2 test / 100 chunks / c=512): quality is a **wash** — perplexity **6.6094** vs **6.6029** for the ROCmFP4 GGUF, inside the ±0.10 error bars — while **prefill ~doubles (~890 → ~1850 tok/s)** and **decode doubles (27.2 → 54.4 tok/s)**. Full matrix in `vault/infrastructure/Analysis/`.
  - ⚠️ **MTP is safe HERE and unsafe on llama.cpp.** Radiance's MTP is **verified distribution-preserving** (greedy output token-identical to unspeculated decoding, 8/8 prompts). The same test on the llama.cpp ROCmFPX build **FAILED — 6/8 diverged mid-content**, as early as character 22. **Never enable `--spec-type draft-mtp` on a llama.cpp path here.** Acceptance rate is a *speed* number and says nothing about this either way.
  - ⚠️ **Context is bounded by a PREALLOCATED KV pool now.** vLLM preallocates; llama.cpp allocated lazily at q4_0. Measured pool ≈ **236,790 tokens**, so 196608 × 1 fits at 1.20× concurrency and the old **327680 × 2 does not fit at any setting**. This is why Hermes's declaration dropped 250000 → **180224** (deliberately UNDER the server's 196608, reserving 16384 tokens of output headroom — declaring the exact ceiling leaves no room for the response, which is why DS4 declares 120000 against a 131072 server).
  - ⚠️ **`compression.threshold: 0.5` in Hermes's config is NOT the effective value.** `_effective_threshold_percent()` (`agent/context_compressor.py`) floors any model under **512K** context at **0.75**, raise-only. Both models here are under it, so the real triggers are `0.75 × declared`: Qwen **135,168**, DS4 **90,000**. The floor exists because compacting a 128–262K model at 50% reclaims less than the incompressible floor (system prompt + tool schemas + protected tail + rolling summary) costs, so compaction re-fires every 1–2 turns and the session spends its wall-clock summarizing. Setting `threshold: 0.75` would be a no-op. To go **above** 0.75, use the per-model map `compression.model_thresholds` (substring keys, longest match wins) — it is **not** global-only.
  - ⚠️ **The old `-fit off` gotcha is gone with llama.cpp; the vLLM equivalent is the kernel line.** Confirm the startup log says **`Using RDNAHybridW4A16LinearKernel`** — without it the AutoRound checkpoint silently lands on vLLM's INC backend or loads **fully unquantized**. `CONFIG_FIX=1` patches the three fields the checkpoint ships wrong (`quant_method`, `desc_act`, `modules_in_block_to_quantize`) and keeps the original as `config.json.autoround-orig`; a re-download reinstates the broken ones.
  - ⚠️ **Cold start is ~7 min**, not ~70 s: the 17.7 GiB checkpoint exceeds free page cache so weights stream from disk every time (`TimeoutStartSec=1800`).
  - `Qwen3.8-27B-Q6_K-R9700-Vulkan-MTP` and `Qwen3.8-27B-ROCmFP4-R9700-Vulkan` (`:8110`) still exist as registry entries but their GGUFs were deleted 2026-08-16 — **radiance is the only Qwen3.8 path on this box.**
