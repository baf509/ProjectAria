...
- Made Next.js build IDs deterministic when `BUILD_SHA` is supplied and stopped
  versioning the generated service worker. Production now generates its cache
  version from the exact deployed revision without dirtying either Git tree.
