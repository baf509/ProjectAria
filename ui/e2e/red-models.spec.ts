...
const M = 'Red-Qwen3.8-27B-PARO-MXFP4'
const F = 'Red-Qwen3.8-Flash-Next-MXFP4'
async function setup(page: Page, opts: { paroLoaded?: boolean; mxfp4Loaded?: boolean; pinned?: string; busy?: boolean; stopFails?: boolean; startFails?: boolean; missing?: boolean; stopped?: boolean; noAdmin?: boolean; unknownActivity?: boolean } = {}) {
  const writes: string[] = []
  let pinned: string | null = opts.pinned ?? C
  const residentRed = opts.mxfp4Loaded ? M : opts.paroLoaded ? P : R
  const servers = [
    { slug: M, state: opts.mxfp4Loaded ? 'running' : 'stopped', startable: true, onbox: false, remotely_operable: true, host_machine: 'machine:red', catalog_visible: true },
    ...
    { slug: R, state: opts.stopped || opts.paroLoaded || opts.mxfp4Loaded ? 'stopped' : 'running', startable: true, onbox: false, remotely_operable: true, host_machine: 'machine:red', catalog_visible: true },
  ...
  await page.route('**/api/v1/**', async route => {
    ...
    else if (path === '/infrastructure/model-servers/utilization') body = { servers: [{ slug: residentRed, reachable: true, busy_slots: opts.unknownActivity ? null : opts.busy ? 1 : 0 }] }
  ...
  await expect(page.getByText(opts.missing ? '4 available' : '5 available', { exact: true })).toBeVisible({ timeout: 45000 })
...
test('switching from PARO MXFP4 to int5 unloads the resident alternative first', async ({ page }) => {
  const x = await setup(page, { mxfp4Loaded: true })
  await page.getByRole('button', { name: 'Switch to Qwen3.8-27B PARO int5', exact: true }).click()
  await expect(page.getByText('Qwen3.8-27B PARO int5 is loaded on Red.', { exact: true })).toBeVisible()
  expect(x.writes).toEqual([`POST /infrastructure/model-servers/${M}/stop`, `POST /infrastructure/model-servers/${P}/start`])
  expect(x.pinned()).toBe(C)
})
