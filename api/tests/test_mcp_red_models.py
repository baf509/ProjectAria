...
async def test_schema_only_offers_supported_choices_and_readonly_status(setup_red):
    ...
    assert set(schema["properties"]["model"]["enum"]) == {"qwen3.8-27b", "qwen-flash-next", "qwen-paro", "qwen3.8-27b-paro-int5"}
    ...
    assert len(status["models"]) == 4 and fleet.posts == []
...
@pytest.mark.asyncio
@pytest.mark.parametrize("choice", ["qwen-paro", "qwen3.8-27b-paro-int5"])
async def test_both_paro_variants_preserve_identity_and_switch_order(setup_red, choice):
    module, _, f, select = setup_red
    f.rows[f.old]["state"] = "running"
    target = module.RED_MODELS[choice]
    result = await select(choice)
    assert result["status"] == "ready" and result["request_model"] == target
    assert f.posts == [(f.old, "stop"), (target, "start")]
