...
Attempts persist on the shell doc, so a periodic Hermes cron sweep gets
three-strikes semantics for free: each sweep nudges a still-paused shell once;
after `shells_nudge_max_attempts` consecutive failed nudges the endpoint
enqueues an alert (source="shells:nudge") that rides the existing alerts →
Hermes triage → Signal path for Ben to confirm/intervene.
...
async def _safety_engaged(db) -> bool:
...
async def nudge_shell(
    ...
    if not request.force and row["idle_seconds"] < settings.shells_nudge_min_idle_seconds:
        return {
            "nudged": False,
    ...
    if last_at is not None:
        ...
        if not request.force and now - last_at < debounce:
    ...
    wait_ms = (
        request.wait_ms if request.wait_ms is not None else settings.shells_nudge_wait_ms
    )
    line, screen = await shell_service.send_input(name, nudge_text, wait_ms=wait_ms)
